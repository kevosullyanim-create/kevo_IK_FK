...
class GenerateIkMatchFkPairsTests(unittest.TestCase):
    ...
    def test_rebuild_generation_keeps_one_ik_handle_pair(self):
        config = {
            "L_arm": {
                "fk_match_ik": [
                ...
                "ik_match_fk": [
                    {"type": "offset", "role": "ik_handle", "ik_ctrl": "ik_ctrl", "source": ""},
                    {"type": "pole_vector", "role": "pole_vector", "ik_ctrl": "pole_ctrl"},
                ],
            }
        }
        ...
        with mock.patch.object(
            ...
            first_build = ikfk_calibrate.generate_ik_match_fk_pairs(
                config,
                "char"
            )
            config["L_arm"]["ik_match_fk"] = first_build["L_arm"]
            rebuilt = ikfk_calibrate.generate_ik_match_fk_pairs(
                config,
                "char"
            )

        handle_pairs = [
            pair for pair in rebuilt["L_arm"]
            if pair.get("role") == "ik_handle"
        ]
        self.assertEqual(1, len(handle_pairs))
        self.assertEqual("ik_ctrl", handle_pairs[0]["ik_ctrl"])
        self.assertEqual("wrist_fk", handle_pairs[0]["source"])
