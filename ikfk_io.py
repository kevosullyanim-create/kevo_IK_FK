...
def _effective_ik_match_fk_type(pair):
...
def _migrate_legacy_ik_controls(ik_match_fk_pairs, ik_controls_entry):
    ...
    for pair in ik_match_fk_pairs:
        ...
        if (
                legacy_ik_ctrl and
                _effective_ik_match_fk_type(pair) == "offset" and
                pair.get("ik_ctrl", "") == legacy_ik_ctrl):
            pair["role"] = "ik_handle"

        if (
                legacy_pole_ctrl and
                _effective_ik_match_fk_type(pair) == "pole_vector" and
    ...
    for pair in ik_match_fk_pairs:
        ...
        if pair.get("role") == "ik_handle" and _effective_ik_match_fk_type(pair) == "offset":
            handle_pair = pair

        if pair.get("role") == "pole_vector" and _effective_ik_match_fk_type(pair) == "pole_vector":
            pole_pair = pair
...
def _dedupe_generated_pair(pairs, role, pair_type, preferred_ctrl):
    matches = [
        ...
        and _effective_ik_match_fk_type(pair) == pair_type
    ]
...
def _normalize_ik_match_fk_pairs(pairs):
    ...
    for pair in pairs:
        ...
        if _effective_ik_match_fk_type(migrated_pair) == "offset":
            ...
            migrated_pair["type"] = "pole_vector"
...
def _read_json_root(path):
    """Read the raw JSON root object at path, or {} if there is no file.

    Used so save helpers can merge into whatever is already on disk instead
    of overwriting unrelated keys. Malformed JSON is deliberately allowed to
    surface to the caller rather than being replaced with an empty config.
    """
...
def save_switch_settings(switch_settings, path):
    ...
    limbs = load_limbs(path) if os.path.isfile(path) else {}
